...
use App\Form\ResetPasswordFromType;
...
class SecurityController extends AbstractController
{
    ...
    public function forgotten_password(Request $request, UserRepository $userRep, TokenGeneratorInterface $tokenGenerator, EntityManagerInterface $manager, SendMailService $mail): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($user) {
                ...
                $mail->send(
                    ...
                    $user->getEmail(),
                    'Réinistialisation mot de passe',
                    'password_reset',
                    $context
                );
    ...
    {
        $user = $userRepo->findOneByRestToken($token);
        if ($user) {
            $form = $this->createForm(ResetPasswordFromType::class);
            $form->handleRequest($request);
            if ($form->isSubmitted() && $form->isValid()) {
                $user->setRestToken('');
